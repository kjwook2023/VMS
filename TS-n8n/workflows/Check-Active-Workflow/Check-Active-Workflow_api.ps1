param(
    ...
    [string]$WorkflowJson = (Join-Path $PSScriptRoot "Check-Active-Workflow_api.json")
)
...
function New-PreparedWorkflowJson {
    ...
    $redactedTeamsWebhook = "https://redacted.invalid/powerautomate/webhook"

    if ($raw.Contains($redactedTeamsWebhook)) {
        if (-not $env:TS_MONITOR_TEAMS_WEBHOOK) {
            throw "TS_MONITOR_TEAMS_WEBHOOK is required when Check-Active-Workflow_api.json contains the redacted Teams webhook placeholder."
        }

        $raw = $raw.Replace($redactedTeamsWebhook, $env:TS_MONITOR_TEAMS_WEBHOOK)
    }

    $tempFile = Join-Path ([System.IO.Path]::GetTempPath()) ("Check-Active-Workflow_" + [guid]::NewGuid().ToString() + ".json")
