param(
    ...
    [string]$WorkflowJson = (Join-Path $PSScriptRoot "Check-n8n-health_api.json")
)
...
function New-PreparedWorkflowJson {
    ...
    if ($raw.Contains($redactedTeamsWebhook)) {
        if (-not $env:TS_MONITOR_TEAMS_WEBHOOK) {
            throw "TS_MONITOR_TEAMS_WEBHOOK is required when Check-n8n-health_api.json contains the redacted Teams webhook placeholder."
        }

        $raw = $raw.Replace($redactedTeamsWebhook, $env:TS_MONITOR_TEAMS_WEBHOOK)
    }

    $tempFile = Join-Path ([System.IO.Path]::GetTempPath()) ("Check-n8n-health_" + [guid]::NewGuid().ToString() + ".json")
