param(
    ...
    [string]$WorkflowJson = (Join-Path $PSScriptRoot "Synchronize-Holiday_api.json")
)
...
if ($target) {
    ...
    $activated = Invoke-N8nApi -Method POST -Url "$baseUrl/api/v1/workflows/$($result.id)/activate"
    [PSCustomObject]@{
        action = "updated"
        id = $activated.id
        name = $activated.name
