...
- Workflow name: TS-IssueCheck(08,16)
- Workflow ID: 73xzOwkeyM0a85Rf
- Active: True
- Archived: False
- Created at: 2026-03-10T23:16:27.595Z
- Updated at: 2026-05-14T00:51:36.000Z
- Version counter: 86
- Node count: 10
...
- IssueCheck: 매주 Mon, Tue, Wed, Thu, Fri 8:00 에 실행되도록 설정되어 있습니다.; 매주 Mon, Tue, Wed, Thu, Fri 16:00 에 실행되도록 설정되어 있습니다.
...
- 휴일체크 -> 기술지원캘린더
- IssueCheck -> 연구소백로그 최신목록조회
- 연구소백로그 최신목록조회 -> Weekday Baseline
- Team용 알림메세지 생성 -> 3.이슈등록관리에 메세지
- 기술지원캘린더 -> Team용 알림메세지 생성
...
- 필요한 이유: 이 노드가 없으면 복잡한 업무 규칙을 기본 노드만으로 표현하기 어렵습니다.
- 해석 근거: type=n8n-nodes-base.code, issue message logic detected
- Credential: 없음
...
- 3.이슈등록관리에 메세지: HTTP API 호출 -> https://redacted.invalid/powerautomate/webhook
- 연구소백로그 최신목록조회: Notion 데이터베이스 조회 -> 연구소 백로그
- 기술지원캘린더: Notion 데이터베이스 조회 -> 기술지원캘린더DB

## 해석 근거

- 워크플로우 이름: TS-IssueCheck(08,16)
...
- Node 연구소백로그 최신목록조회 -> notionApi / tsupport API (LrLyYGhPvHMbPfZm)
...
- JSON payload: TS-IssueCheck(08,16)_api.json
- PowerShell upsert script: TS-IssueCheck(08,16)_api.ps1
